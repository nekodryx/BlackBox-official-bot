module.exports = {
  ...
  duration: function(ms) {
      ...
      return `\`${days}Dní\`,\`${hrs}Houdin\`,\`${min}Minut\`,\`${sec}Sekund\``;
  },
  ...
  format: function(millis) {
    try{
      ...
      if (h < 1) return (m < 10 ? "0" : "") + m + ":" + (s < 10 ? "0" : "") + s + " | " + (Math.floor(millis / 1000)) + " Sekund";
      else return (h < 10 ? "0" : "") + h + ":" + (m < 10 ? "0" : "") + m + ":" + (s < 10 ? "0" : "") + s + " | " + (Math.floor(millis / 1000)) + " Sekund";
    }catch (e){
      console.log(String(e.stack).bgRed)
    }
  },
